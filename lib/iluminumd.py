...
class IluminumDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some iluminumd methods
    ...
    @classmethod
    def from_iluminum_conf(self, iluminum_dot_conf):
        from iluminum_config import IluminumConfig
        config_text = IluminumConfig.slurp_config_file(iluminum_dot_conf)
        creds = IluminumConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = iluminumlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = iluminumlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = iluminumlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_iluminumd_proto_version_with_sentinel_ping)
