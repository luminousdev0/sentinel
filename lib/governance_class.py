...
class GovernanceClass(object):
    ...
    def vote(self, iluminumd, signal, outcome):
        return self.go.vote(iluminumd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, iluminumd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(iluminumd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(iluminumd, models.VoteSignals.valid, models.VoteOutcomes.no)
    ...
    def list(self):
        ...
        # return a dict similar to iluminumd "gobject list" output
    ...
    def iluminumd_serialise(self):
        import iluminumlib
        return iluminumlib.SHIM_serialise_for_iluminumd(self.serialise())
