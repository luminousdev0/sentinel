...
def is_valid_iluminum_address(address, network='mainnet'):
    ...
    iluminum_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Iluminum address
        return False

    if (address_version != iluminum_version):
        return False
...
# shims 'til we can fix the iluminumd side
def SHIM_serialise_for_iluminumd(sentinel_hex):
    ...
    # superblock => "trigger" in iluminumd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # iluminumd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    iluminumd_hex = serialise(obj)
    return iluminumd_hex


# shims 'til we can fix the iluminumd side
def SHIM_deserialise_from_iluminumd(iluminumd_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(iluminumd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('iluminum.conf')
