...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, iluminumd):
        golist = iluminumd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_iluminumd(iluminumd, item)
    ...
    @classmethod
    def import_gobject_from_iluminumd(self, iluminumd, rec):
        import decimal
        import iluminumlib
        import inflection
        ...
        # shim/iluminumd conversion
        object_hex = iluminumlib.SHIM_deserialise_from_iluminumd(object_hex)
        objects = iluminumlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from iluminumd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from iluminumd, with every run
        ...
        # get/create, then sync payment amounts, etc. from iluminumd - Iluminumd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(iluminumd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from iluminumd! %s" % e)
            govobj.vote_delete(iluminumd)
            return (govobj, None)
    ...
    def vote_delete(self, iluminumd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(iluminumd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, iluminumd, signal, outcome):
        import iluminumlib
        ...
        output = iluminumd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = iluminumlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(iluminumd, signal)

    def sync_network_vote(self, iluminumd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = iluminumd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 iluminum addr, non-multisig
            if not iluminumlib.is_valid_iluminum_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Iluminum address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (iluminumlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/IluminumDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import iluminumlib
        obj_data = iluminumlib.SHIM_serialise_for_iluminumd(self.serialise())
    ...
    def prepare(self, iluminumd):
        try:
            object_hash = iluminumd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not iluminumlib.is_valid_iluminum_address(addr, config.network):
    ...
    def hash(self):
        import iluminumlib
        return iluminumlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, iluminumd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - iluminumd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, iluminumd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - iluminumd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, iluminumd):
        now = int(time.time())
        return (self.created_at < (now - iluminumd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, iluminumd):
        if self.is_expired(iluminumd):
            return False

        return True

    def is_deletable(self, iluminumd):
        if self.is_expired(iluminumd):
            return True
