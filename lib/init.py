...
def has_iluminum_conf():
    import config
    import io

    valid_iluminum_conf = False

    # ensure iluminum_conf exists & readable
    #
    # if not, print a message stating that Iluminum Core must be installed and
    # configured, including JSONRPC access in iluminum.conf
    try:
        f = io.open(config.iluminum_conf)
        valid_iluminum_conf = True
    except IOError as e:
        print(e)

    return valid_iluminum_conf
...
def main():
    ...
    if not has_iluminum_conf():
        print("IluminumCore must be installed and configured, including JSONRPC access in iluminum.conf")
        sys.exit(1)
