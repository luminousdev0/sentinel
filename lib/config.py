...
sentinel_cfg = IluminumConfig.tokenize(sentinel_config_file)
sentinel_version = "1.1.0"
min_iluminumd_proto_version_with_sentinel_ping = 70207


def get_iluminum_conf():
    home = os.environ.get('HOME')

    iluminum_conf = os.path.join(home, ".iluminumcore/iluminum.conf")
    if sys.platform == 'darwin':
        iluminum_conf = os.path.join(home, "Library/Application Support/IluminumCore/iluminum.conf")

    iluminum_conf = sentinel_cfg.get('iluminum_conf', iluminum_conf)

    return iluminum_conf
...
iluminum_conf = get_iluminum_conf()
network = get_network()
db = get_db_conn()
